...
class RecordCollab(object):
    ...
    def request_download(self, recd):
        if recd.meshDownloading:
            logger.debug('meshInitRoundRobin: we are in midst of downloading'
                         ' this file...')
            return
    ...
    def _setup(self):
        ...
        bus_name, conn_path, channel_paths = \
            self.activity._shared_activity.get_channels()
        room = None
        tubes_chan = None
        text_chan = None
    ...
    def _new_tube_cb(self, id, initiator, type, service, params, state):
        logger.debug(
            'New tube: '
            'ID=%d initator=%d type=%d service=%s params=%r state=%d',
    ...
    def _req_recd_from_buddy(self, recd, sender, nick):
        ...
        recd.meshReqCallbackId = gobject.timeout_add(
            self._collab_timeout, self._check_recd_request, recd)
    ...
    def _next_round_robin_buddy(self, recd):
        ...
        for buddy_obj in buds:
            ...
            if recd.triedMeshBuddies.count(buddy) > 0:
                logger.debug(
                    'mnrrb: weve already tried bud ' + buddy_obj.props.nick)
            else:
    ...
    def _recd_request_cb(self, remote_object, remote_person, md5sum):
        # if we are here, it is because someone has been told
        ...
        if recd.deleted:
            logger.debug('_recdRequestCb: we have the recd, '
                         'but it has been deleted, so we wont share')
            self._tube.unavailableRecd(md5sum, Instance.keyHashPrintable,
                                       remote_person)
            return
        ...
        if recd.buddy and not recd.downloadedFromBuddy:
            logger.debug(
                '_recdRequestCb: we have an incomplete recd, so we wont share')
            self._tube.unavailableRecd(md5sum, Instance.keyHashPrintable,
                                       remote_person)
            return
    ...
    def _check_recd_request(self, recd):
        ...
        if recd.meshDownloadingProgress:
            ...
            # we've received some bits since last we checked,
            # so keep waiting...  they'll all get here eventually!
            recd.meshDownloadingProgress = False
            return True
        else:
            logger.debug('_meshCheckOnRecdRequest: ! '
    ...
    def _recd_bits_arrived_cb(self, remote_object, md5sum, part, num_parts,
        ...
        if recd.deleted:
            ...
            return
        if recd.downloadedFromBuddy:
            logger.debug('_recdBitsArrivedCb:'
                         ' weve already downloadedFromBuddy')
            return
        if not recd.buddy:
        ...
        if recd.meshDownloadingFrom != sender:
            logger.debug('_recdBitsArrivedCb: wrong bits $s, exp %s',
                         str(sender), str(recd.meshDownloadingFrom))
            return
    ...
    def _recd_unavailable_cb(self, remote_object, md5sum, sender):
        ...
        recd = self.model.get_recd_by_md5(md5sum)
        if not recd:
            logger.debug('_recdUnavailableCb: actually, '
                         'we dont even know about that one..')
            return
        if recd.deleted:
            logger.debug('_recdUnavailableCb: actually, '
                         'since we asked, we deleted.')
            return
        if not recd.buddy:
        ...
        if recd.downloadedFromBuddy:
            ...
            return
        if recd.meshDownloadingFrom != sender:
            logger.debug('_recdUnavailableCb: we arent asking you for a copy'
                         ' now.  slow response, pbly.')
            return
