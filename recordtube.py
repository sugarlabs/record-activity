...
class RecordTube(ExportedGObject):
    ...
    def notifyBudsOfNewRecd(self, recorder, recdXml):
        logger.debug(
            'Ive taken a new pho-ideo-audio! '
            'I hereby send you an xml thumb of said media via this interface.')
    ...
    def broadcastRecd(self, md5, filepath, sendThisTo):
        ...
        for chunk in range(chunks):
            ...
            if chunk == 0:
                logger.debug("sending %s of %s to %s", (chunk + 1), chunks,
                             sendThisTo)
            if chunk == chunks - 1:
                logger.debug("sending %s of %s to %s", (chunk + 1), chunks,
                             sendThisTo)
    ...
    def _getRecdTubeCb(self, md5, part, numparts, bytes, sentTo, fromWho,
        ...
        if (fromWho == Instance.keyHashPrintable):
            # record.Record.log.debug('_getRecdTubeCb:
            # i dont want bits from meself, thx anyway.  schizophrenic?')
            return
        if (sentTo != Instance.keyHashPrintable):
            # record.Record.log.debug('_getRecdTubeCb:
            # ive overhead someone sending bits, but not to me!')
            return
    ...
    def unavailableRecd(self, md5sumOfIt, whoDoesntHaveIt, whoAskedForIt):
        logger.debug('unavailableRecd: id love to share this photo,'
    ...
    def _unavailableRecdTubeCb(self, md5sumOfIt, whoDoesntHaveIt,
        ...
        if whoDoesntHaveIt == Instance.keyHashPrintable:
            logger.debug('_unavailableRecdTubeCb: yes, i know i dont have it,'
                         ' i just told you/me/us.')
            return
        if whoAskedForIt != Instance.keyHashPrintable:
            logger.debug(
                '_unavailableRecdTubeCb: ive overheard someone '
                'doesnt have a photo, but i didnt ask for that one anyways')
            return
